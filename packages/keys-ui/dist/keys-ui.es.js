    ...
    return (e) => {
      ...
      switch (s) {
        ...
        case "Escape":
          ...
          break;
        case "Home":
          a && e.preventDefault(), (p = t.onHome) == null || p.call(t);
          break;
        case "End":
...
let g = M;
typeof window < "u" && (window.AnimationUtils = g);
class L extends y {
  /**
   ...
   */
  dismissAlert(t) {
    t.classList.add("alert-dismissing"), g.slideOut(t, "right", {
      duration: 300,
      easing: "ease-out",
      distance: 100
    }), g.collapseHeight(t, {
   ...
   */
  showAlert(t) {
    t.style.display = "block", g.slideIn(t, "right", {
      duration: 300,
      easing: "ease-out",
      distance: 100,
      onComplete: () => {
...
class w extends y {
  ...
  bindEventListeners() {
    ...
    }), u.handleDelegatedClick("[data-calendar-action]", (t, e) => {
      if (!t.disabled) {
        e.preventDefault();
        const s = r.findClosest(t, '[data-calendar="true"]'), a = t.dataset.calendarAction;
        s && !this.isCalendarDisabled(s) && this.handleFooterAction(s, a);
      }
  ...
  generateCalendarGrid(t) {
    ...
    for (; c <= l; ) {
      const h = this.formatDateString(c), p = c.getMonth() === a - 1 && c.getFullYear() === s, m = {
        date: h,
        day: c.getDate(),
        isCurrentMonth: p,
  ...
  focusDate(t, e) {
    ...
    s.focusedDate = e, this.setState(t, s), g.createTimer(() => {
  ...
  animateViewTransition(t, e) {
    t.style.opacity = "0.7", t.style.transform = "scale(0.98)", g.createTimer(() => {
  ...
  renderMultipleMonths(t) {
    ...
    s.forEach((i, n) => {
      ...
      }, d = this.generateCalendarGridForMonth(t, l, n, s.length);
      ...
      c += '<table class="w-full" role="grid" aria-label="Calendar">', c += '<thead><tr role="row">', e.weekdays.forEach((f) => {
        const p = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][e.weekdays.indexOf(f)];
        c += `<th class="calendar-weekday text-center text-muted font-medium p-1" role="columnheader" aria-label="${p}">${f}</th>`;
      }), c += "</tr></thead>", c += "<tbody>", d.forEach((f) => {
        c += '<tr role="row">', f.forEach((h) => {
          const p = this.getDayButtonClasses(h), m = this.getDateAriaLabel(h.date, h.isToday, h.isSelected, h.isRangeStart, h.isRangeEnd, h.isInRange), S = this.getRangeAttributes(h, e);
          c += `
                        <td class="calendar-day text-center relative" role="gridcell">
                            <button type="button"
                                    class="${p}"
   ...
   */
  generateCalendarGridForMonth(t, e, s = 0, a = 1) {
    const i = this.getState(t);
    if (!i) return [];
    this.setState(t, { ...i, currentMonth: e.currentMonth });
    let n = this.generateCalendarGrid(t);
    return a > 1 && s < a - 1 && n[n.length - 1].some((d) => !d.isCurrentMonth && d.day <= 15) && (n = n.slice(0, -1)), this.setState(t, i), n;
  }
  /**
  ...
  navigateToToday(t) {
    const e = this.getState(t);
    if (!e) return;
    const s = /* @__PURE__ */ new Date(), a = `${s.getFullYear()}-${String(s.getMonth() + 1).padStart(2, "0")}`;
    e.currentMonth !== a && (e.currentMonth = a, this.setState(t, e), this.renderCalendarDisplay(t));
  }
  /**
...
class O extends y {
  ...
  openDropdown(t) {
    ...
    i && t.dataset.searchable === "true" && g.createTimer(() => i.focus(), 10), this.updateFilteredOptions(t), this.dispatchSelectEvent(t, "select:open");
  }
  /**
   * Close dropdown
   */
...
class E extends y {
  ...
  dismiss(t) {
    ...
    return s ? (this.clearTimer(t), e.pausedTimers.delete(t), s.setAttribute("data-toast-visible", "false"), s.setAttribute("data-toast-exiting", "true"), g.fadeOut(s, {
      scale: !0,
      duration: 300,
      onComplete: () => {
  ...
  clearTimer(t) {
    ...
    s && (g.clearTimer(s), e.timers.delete(t));
  }
  /**
  ...
  resumeTimer(t) {
    ...
    s && a ? s.getAttribute("data-toast-persistent") === "true" || (g.resumeTimer(a), e.pausedTimers.delete(t)) : s && i && !(s.getAttribute("data-toast-persistent") === "true") && i.remaining > 0 && (this.setTimer(t, i.remaining), e.pausedTimers.delete(t));
  }
  /**
  ...
  onDestroy() {
    const t = this.getGlobalState();
    t && (t.timers.forEach((e) => g.clearTimer(e)), t.timers.clear(), t.pausedTimers.clear(), t.toasts.forEach((e) => {
...
class q extends y {
  ...
  positionDropdown(t) {
    ...
    o - a.left, a.right;
    let p = l, m = d;
    switch (l === "bottom" && f < i.height && h > i.height ? p = "top" : l === "top" && h < i.height && f > i.height && (p = "bottom"), p) {
      case "top":
  ...
  positionSubmenu(t) {
    ...
    n - a.bottom, a.top;
    let p = l;
    switch (l === "right" && f < i.width && h > i.width ? p = "left" : l === "left" && h < i.width && f > i.width && (p = "right"), p) {
      case "right":
        e.style.left = "100%", e.style.marginLeft = `${c}px`;
        break;
      case "left":
...
class x extends y {
  ...
  scheduleShow(t) {
    const e = this.getState(t);
    !e || t.getAttribute("data-disabled") === "true" || (this.cancelHide(t), e.showTimer = g.createTimer(() => {
      this.showTooltip(t);
    }, e.delay));
  }
  /**
  ...
  scheduleHide(t) {
    const e = this.getState(t);
    e && (this.cancelShow(t), e.hideTimer = g.createTimer(() => {
      this.hideTooltip(t);
    }, 100));
  }
  /**
  ...
  cancelShow(t) {
    const e = this.getState(t);
    e != null && e.showTimer && (g.clearTimer(e.showTimer), delete e.showTimer);
  }
  /**
  ...
  cancelHide(t) {
    const e = this.getState(t);
    e != null && e.hideTimer && (g.clearTimer(e.hideTimer), delete e.hideTimer);
  }
  /**
   * Show tooltip
   */
  showTooltip(t) {
    const e = this.getState(t);
    !e || e.isVisible || (e.trigger && this.positionTooltip(e.trigger, t), g.fadeIn(t, {
      duration: 200,
      onComplete: () => {
...
class T extends y {
  ...
  positionDropdown(t) {
    ...
    let h = !1, p = "none";
    l <= d ? h = !1 : l <= c ? h = !0 : c > d ? (h = !0, p = Math.max(c, f) + "px") : (h = !1, p = Math.max(d, f) + "px"), h ? (e.style.bottom = "100%", e.style.top = "auto", e.style.marginBottom = "4px", e.style.marginTop = "0") : (e.style.top = "100%", e.style.bottom = "auto", e.style.marginTop = "4px", e.style.marginBottom = "0"), p !== "none" && (e.style.maxHeight = p, e.style.overflowY = "auto");
    const m = a.left;
...
class C extends y {
  ...
  shrink(t) {
    ...
    e.animation = g.collapseHeight(t, {
      toHeight: a,
      duration: 300,
      easing: "ease-out",
      onComplete: () => {
  ...
  openAccordion(t) {
    ...
    if (e.open)
      return !0;
    const s = this.getState(e);
    return s && s.isAnimating ? !1 : g.prefersReducedMotion() ? (e.open = !0, !0) : (this.expand(e), !0);
  }
  /**
  ...
  closeAccordion(t) {
    ...
    if (!e.open)
      return !0;
    const s = this.getState(e);
    return s && s.isAnimating ? !1 : g.prefersReducedMotion() ? (e.open = !1, !0) : (this.shrink(e), !0);
  }
  /**
  ...
  onDestroy() {
    this.getAllStates().forEach((t, e) => {
      g.cancelAnimation(t.animation), e.removeAttribute("animating"), e.style.height = "", e.style.overflow = "";
    });
  }
}
document.readyState === "loading" ? document.addEventListener("DOMContentLoaded", () => {
